const translations = {
  en: {
    "hero-title": "Fast and reliable auto locksmith service",
    "hero-sub":
      "Car keys, motorcycles, vans, lorries – programming, duplication, ECU remapping, emergency unlocking",
    "banner-text": "🔐 24/7 Service throughout Lithuania  .",
    "hero-phone": "📞 Call +370 623 85036  or WhatsApp",
    "contact-button": "📞Call Now",
    "contact-aria": "Call us now",
    "about-title": "About Us",
"about-text": `
<p>RTi Auto Raktai has over <strong>10 years of experience</strong> in the automotive locksmith industry.<br>
We specialize in a wide range of services including:</p>
<ul style="margin-left: 4rem; list-style-position: outside;">
  <br>
  <strong>
  <li>Vehicle key programming</li>
  <li>ECU remapping and immobilizer solutions</li>
  <li>Emergency unlocking for all makes and models</li></strong><br>
</ul>
<p>We provide fast, responsible, and professional service for <strong>cars, vans, motorcycles, and trucks</strong>.<br>
When you need reliable help with vehicle keys – <strong>we’re the team you can trust</strong>.</p>
`,
"services-title": "Our Services",
"service1": "Car key programming",
"service2": "Motorcycle key cutting and programming",
"service3": "Truck and van key solutions",
"service4": "Emergency door unlocking",
"service5": "ECU and chip programming",
"service6": "Immobiliser removal and configuration",
 motorbikeService: "<strong>Motorbike keys</strong> <br><br>We program and make keys for Yamaha, Honda, Suzuki, KTM and more. Immo off, ignition lock repairs.",
    truckService: "<strong>Truck keys</strong> <br><br>Key duplication, immobilizer programming, emergency unlocking. We work with MAN, Volvo, DAF, Mercedes and others.",
    vanService: "<strong>Van keys</strong> <br><br>Van key production and pairing with the onboard computer. Popular models: Renault Trafic, VW Transporter, Ford Transit.",
    autoService: "<strong>Car keys</strong> <br><br>Car key duplication, immobilizer programming, emergency unlocking. We work with BMW, Audi, Volkswagen, Ford, Volvo, and others.",
    postalService: `
    <img src="images/mail-global.png" alt="Mail-in lock service" style="max-width: 50px; border-radius: 8px; margin-bottom: 20px;">
      <h2>Mail-in Service</h2>
      <p>We offer a unique <strong>mail-in service for lock repair, programming, and key duplication</strong>. Simply send us your car or motorcycle lock – we’ll repair it, reprogram it, and ship it back.</p>
      <a href="docs/mail-in-form.pdf" target="_blank" class="btn">📄 Download Order Form (PDF)</a>`,
    "gallery-title": "Gallery",
    "contact-title": "Contact",
    "training-title": "Training",
    "training-text": `
RTi Auto Raktai also provides professional training in car key programming, duplication, ECU remap, and emergency unlocking.<br><br>
Our courses are designed for both <strong>beginners</strong> and <strong>experienced auto locksmiths</strong> who want to deepen their skills.<br>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li> Hands-on practice with real vehicles and equipment</li>
  <li> Theory and diagnostics (OBD, EEPROM, transponder systems)</li>
  <li> Certification upon completion</li>
  <li> Training available online or in person</li>
  <li> Multilingual: Lithuanian, English, Russian, Polish</li></strong>
</ul>
<br>`,
  
"chipTuning.title": "Chip Tuning",
    "chipTuning.description": "Professional OBD diagnostics for cars and motorcycles.",
    "chipTuning.servicesTitle": "Available chip tuning options:",
    "chipTuning.stage1": "Stage 1 – power increase",
    "chipTuning.eco": "Eco tuning – fuel saving",
    "chipTuning.note": "We perform reliable tuning with the latest equipment.",

    "lockout-title": "Emergency Car Unlocking",
"lockout-text": `
<p>Locked out of your car? It can happen to anyone – usually at the worst possible moment.<br><strong>We’re ready to help!</strong></p>
<p>Our qualified technicians respond quickly, covering <strong>all across Lithuania</strong>, day or night.</p>
<p>We use <strong>safe and non-destructive</strong> unlocking methods suitable even for the newest vehicles with advanced security systems.</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Fast response throughout Lithuania</li>
  <li>Non-damaging, professional techniques</li>
  <li>Cars, vans, motorcycles</li>
  <li>24/7 availability</li>
  <li>Experienced, professional technicians</li>
  <li>Multilingual support</li></strong>
</ul>
`,

   "key-replacement-title": "Car Key Replacement",
"key-replacement-text": `
<p>Lost or broken your car key? Don’t worry – RTi Auto Raktai is here to help.<br>
We specialize in <strong>on-site replacement</strong> of all types of vehicle keys, including:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li> Remote keys</li>
  <li> Smart and proximity keys</li>
  <li> Transponder keys</li>
  <li>Retro and classic car keys</li><strong><br>
</ul>
<p>Our mobile experts can <strong>cut, clone, and program</strong> your new key while you wait – at your home, work, or roadside location.<br></p>
`,

    "why-us-title": "Why Choose Us?",
"why-us-text": `
<p>We’re more than just locksmiths – we’re your trusted vehicle security partner.<br>
At RTi Auto Raktai, we combine <strong>top-quality service</strong> with <strong>affordable pricing</strong> and local accessibility.</p>

<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li> 5-star rated by hundreds of happy customers</li>
  <li> Transparent and competitive prices – no hidden fees</li>
  <li> Mobile technicians available across Lithuania</li>
  <li> 24/7 emergency support</li>
  <li> Expertise with all makes and models</li>
  <li> Friendly, multilingual and professional approach</li></strong>
</ul>

<p>When you choose us, you get <strong>peace of mind</strong>, <strong>speedy assistance</strong>, and <strong>proven results</strong>.</p>
`,
"ignition-title": "Ignition Lock Repair",
"ignition-text": `
<p>The ignition system is one of the most important and sensitive components of your vehicle.<br><br>
At RTi Auto Raktai, we offer professional services to:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Repair faulty or worn-out ignition cylinders</li>
  <li>Replace damaged or non-functioning ignition locks</li>
  <li>Extract broken keys from ignition slots</li></strong>
</ul>
<p><strong>Never attempt ignition repair on your own</strong> – incorrect handling can cause serious damage.<br><br>
Trust our experienced technicians to restore your vehicle’s ignition safely and efficiently.</p>
`,

    "programming-title": "Car Key Programming",
"programming-text": `
<p>Modern vehicles require advanced technology for key programming. At RTi Auto Raktai, we use the latest equipment to ensure precision and security.<br><br>
Our programming services include:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>OBD-based key programming</li>
  <li>EEPROM-level chip programming</li>
  <li>Transponder key pairing</li>
  <li>ECU and immobilizer synchronization</li></strong>
</ul>
<p>Whether you need a simple spare key or complete electronic integration – <strong>we deliver accurate and reliable results</strong>.</p>
`,
"pricing-title": "Pricing & Services",
"pricing-text": `
<p>We believe in clear and competitive pricing – no hidden fees, no surprises.<br><br>
Below is a general guide to our service rates:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Lockout service – from €20</li>
  <li>Lost key solutions – from €80</li>
  <li>Key repairs – from €30</li>
  <li>Key duplication – from €40</li>
  <li>Remote keys and ignition lock repair – from €100</li></strong>
</ul><br>
<p>All prices include professional service and warranty. Contact us for an exact quote based on your vehicle and situation.</p>
`,

    "contact-us-title": "Contact Us",
    "contact-us-text":
      "Available 24/7 across Lithuania, WhatsApp or fill out the form to reach us.",
    "uk-contact": "📍 Lithuania, Anykščiai district ",
    footer: "© 2025 RTi Auto Raktai. All rights reserved.",
    name: "Your name",
    email: "Your email",
    message: "Your message",
    "send-button": "Send",
    required: "Please fill in this field.",
    invalidEmail: "Please enter a valid email address.",
    thanksTitle: "Thank you!",
    thanksMsg: "Your message has been submitted.",
    "nav-chip-tuning": "Chip Tuning",
    "nav-postal": "Mail-in Service",
    "nav-about": "About Us",
    "nav-services": "Our Services",
    "nav-lockout": "Car Lockout Service",
    "nav-key-replacement": "Car Key Replacement",
    "nav-why-us": "Why Choose Us?",
    "nav-ignition": "Ignition Lock Repair",
    "nav-programming": "Car Key Programming",
    "nav-pricing": "Pricing & Services",
    "nav-training": "Training",
    "nav-contact": "Contact",
    "disclaimer-title": "Disclaimer Form",
    "full-name-label": "Full Name",
    "email-label": "Email",
    "vehicle-details-label": "Vehicle Details",
    "disclaimer-text-label": "Disclaimer",
    "disclaimer-text": "RTi Auto Raktai is not responsible for any issues that occur after key programming or creation. I confirm that I was informed about possible risks, and I release the company from all liability.",
    "agree-check": "I agree to the terms and conditions",
    "submit-button": "Submit",
    "thanks-title": "Thank You!",
    "thanks-text": "Your disclaimer has been submitted successfully.",
  },
  lt: {
    "hero-title": "Greita ir patikima automobilių raktų pagalba",
    "hero-sub":
      "Automobiliai, motociklai, autobusiukai, vilkikai – programavimas, kopijavimas, ECU Čipų programavimas, avarinis atrakinimas",
    "banner-text": "🔐 24/7 pagalba visoje Lietuvoje.",
    "contact-button": " 📞Skambinti",
    "hero-phone": "📞 Skambinkite +370 623 85036 arba rašykite WhatsApp",
    "contact-button": "Skambinti",
     "contact-aria": "Skambinkite mums dabar",
    "about-title": "Apie mus",
"about-text": `
<p>RTi Auto Raktai turi daugiau nei <strong>10 metų patirties</strong> automobilių raktų programavimo ir avarinio atrakinimo srityje.<br><br>
Mūsų specializacija apima:</p>
<ul style="margin-left: 4rem; list-style-position: outside;">
  <br><strong>
  <li>Transporto priemonių raktų programavimą</li>
  <li>ECU  čipų ir imobilaizerių sprendimus</li>
  <li>Avarinį visų transporto priemonių atrakinimą</li></strong><br>
</ul>
<p>Dirbame greitai, atsakingai ir profesionaliai su <strong>lengvaisiais automobiliais, motociklais, mikroautobusais ir vilkikais</strong>.<br><br>
Kai reikia patikimos pagalbos dėl automobilio raktų – <strong>kreipkitės į mus</strong>.</p>
`,
    "services-title": "Mūsų paslaugos",
"service1": "Automobilių raktų programavimas",
"service2": "Motociklų raktų gamyba ir programavimas",
"service3": "Vilkikų raktų sprendimai",
"service4": "Avarinis durų atrakinimas",
"service5": "ECU ir čipų programavimas",
"service6": "Imobilaizerių šalinimas ir pritaikymas",
    motorbikeService: "<strong>Motociklų raktai</strong> <br><br>Programuojame ir gaminame motociklų raktus: Yamaha, Honda, Suzuki, KTM ir kt. Immo išjungimas, užvedimo spynelių remontas.",
    truckService: "<strong>Sunkvežimių raktai</strong> <br><br>Raktų dublikavimas, imobilizatoriaus programavimas, avarinis atrakinimas. Dirbame su MAN, Volvo, DAF, Mercedes ir kt.",
    vanService: "<strong>Mikroautobusų raktai</strong> <br><br>Mikroautobusų raktų gamyba ir pririšimas prie borto kompiuterio. Visi populiariausi modeliai: Renault Trafic, VW Transporter, Ford Transit.",
    autoService: "<strong>Automobilių raktai</strong> <br><br>Automobilių raktų dublikavimas, imobilizatoriaus programavimas, avarinis atrakinimas. Dirbame su BMW, Audi, Volkswagen, Ford, Volvo, ir kt.",
    postalService: `
    <img src="images/mail-global.png" alt="Pašto paslaugos" style="max-width: 50px; border-radius: 8px; margin-bottom: 20px;">
    <h2>Užsakymas paštu</h2>
    <p>Siūlome unikalią paslaugą – <strong>spynų remontą, programavimą ir raktų gamybą paštu</strong>. Jums tereikia atsiųsti automobilio ar motociklo spyną mums – mes ją suremontuosime, priprogramuosime ir grąžinsime atgal.</p>
    <a href="docs/mail-in-form.pdf" target="_blank" class="btn">📄 Atsisiųsti užsakymo formą (PDF)</a>
  `,
    "gallery-title": "Galerija",
    "contact-title": "Kontaktai",
    "uk-contact": "📍 Lietuva, Anykščių rajonas",

    "training-title": "Mokymai",
    "training-text": `
RTi Auto Raktai taip pat teikia profesionalius mokymus automobilių raktų programavimo, kopijavimo, ECU Čipų programavimas ir avarinio atrakinimo srityse.<br>
Mūsų kursai skirti tiek <strong>pradedantiesiems</strong>, tiek <strong>patyrusiems auto spynų meistrams</strong>, norintiems pagilinti savo žinias.<br><br>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong><br>
  <li> Praktiniai užsiėmimai su realiais automobiliais ir įranga</li>
  <li> Teorija ir diagnostika (OBD, EEPROM, transponderių sistemos)</li>
  <li> Sertifikatas po mokymų</li>
  <li> Mokymai vyksta nuotoliniu būdu arba gyvai</li>
  <li> Kalbos: lietuvių, anglų, rusų, lenkų</li></strong>
</ul>
<br>
`,
   "chipTuning.title": "Chip Tuning",
    "chipTuning.description": "Profesionali automobilių ir motociklų OBD kompiuterinė diagnostika.",
    "chipTuning.servicesTitle": "Galimi chip tuning sprendimai:",
    "chipTuning.stage1": "Stage 1 – galios didinimas",
    "chipTuning.eco": "Eco tuning – kuro taupymas",
    "chipTuning.note": "Naudojame naujausią įrangą, dirbame atsakingai ir profesionaliai.",

   "lockout-title": "Avarinis automobilio atrakinimas",
"lockout-text": `
<p>Netikėtai užrakintas automobilis – tai stresas, kuris gali ištikti kiekvieną, dažniausiai netinkamiausiu metu.<br><strong>Mes pasiruošę padėti!</strong></p>
<p>Mūsų kvalifikuoti specialistai atvyksta greitai, dirba <strong>visoje Lietuvoje</strong> ir padeda tiek dieną, tiek naktį.</p>
<p>Naudojame <strong>saugias ir neardomas</strong> atrakinimo technologijas, kurios tinka net naujausiems automobilių modeliams su aukšto lygio apsauga.</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Greitas iškvietimas visoje Lietuvoje</li>
  <li>Nežalingi, profesionalūs atrakinimo metodai</li>
  <li>Tinka automobiliams, mikroautobusams ir motociklams</li>
  <li>Pagalba 24/7 – visomis savaitės dienomis</li>
  <li>Patyrę ir atsakingi specialistai</li>
  <li>Aptarnavimas keliomis kalbomis</li></strong>
</ul>
`,
    "key-replacement-title": "Automobilio rakto keitimas",
"key-replacement-text": `
<p>Praradote ar sulaužėte automobilio raktą? Nesijaudinkite – RTi Auto Raktai pasiruošę padėti.<br>
Mes specializuojamės <strong>vietoje atliekamame rakto keitime</strong> ir siūlome šiuos sprendimus:</p><br> <br>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li> <strong>Pulteliai</strong> (remote)</li>
  <li> <strong>Išmanieji</strong> (smart) ir bekontakčiai raktai</li>
  <li> <strong>Transponderiniai</strong> raktai</li>
  <li> <strong>Retro</strong> ir klasikiniai raktai</li><br><strong>
</ul>
<p>Mūsų specialistai gali <strong>išpjauti, nukopijuoti ir suprogramuoti</strong> naują raktą vietoje – jūsų namuose, darbe ar kelyje.<br>
</p>
`,

"why-us-title": "Kodėl verta rinktis mus?",
"why-us-text": `
<p>Mes ne tik spynų meistrai – mes jūsų patikimas partneris automobilių saugumo srityje.<br><br>
RTi Auto Raktai siūlo <strong>aukščiausio lygio paslaugas</strong> už <strong>prieinamą kainą</strong>, visada netoliese.</p>

<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li> Šimtai 5 žvaigždučių įvertinimų</li>
  <li> Skaidrios ir konkurencingos kainos – jokių paslėptų mokesčių</li>
  <li>Mobilūs meistrai Rytų ir Vidurio Lietuvoje</li>

  <li> Pagalba 24/7 – bet kada, bet kur</li>
  <li> Dirbame su visų markių ir modelių transportu</li>
  <li> Profesionalus, malonus ir daugiakalbis aptarnavimas</li></strong>
</ul>

<p>Pasirinkdami mus, gaunate <strong>ramybę</strong>, <strong>greitą pagalbą</strong> ir <strong>tikrus rezultatus</strong>.</p>
`,

   "ignition-title": "Užvedimo spynos remontas",
"ignition-text": `
<p>Užvedimo spynos – viena jautriausių ir svarbiausių automobilio dalių.<br><br>
RTi Auto Raktai profesionaliai atlieka šias paslaugas:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Užvedimo spynelių remontas ir nusidėvėjimo šalinimas</li>
  <li>Pažeistų ar neveikiančių spynų keitimas</li>
  <li>Nulūžusių raktų ištraukimas iš spynos</li></strong>
</ul>
<p><strong>Nebandykite remontuoti savarankiškai</strong> – neteisingas veiksmas gali sugadinti visą sistemą.<br><br>
Mūsų patyrę meistrai pasirūpins, kad jūsų Užvedimo sistema veiktų saugiai ir patikimai.</p>
`,

   "programming-title": "Automobilio rakto programavimas",
"programming-text": `
<p>Šiuolaikiniai automobiliai reikalauja pažangių sprendimų raktų programavimui. RTi Auto Raktai naudoja modernią įrangą ir užtikrina saugų bei tikslų darbą.<br><br>
Mūsų programavimo paslaugos apima:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Programavimą per OBD jungtį</li>
  <li>EEPROM mikroschemų programavimą</li>
  <li>Transponderio rakto suporavimą</li>
  <li>ECU ir imobilaizerio sinchronizavimą</li></strong>
</ul>
<p>Ar reikia paprasto atsarginio rakto, ar visos elektroninės sistemos integracijos – <strong>mes užtikriname tikslų ir patikimą rezultatą</strong>.</p>
`,

   "pricing-title": "Kainos ir paslaugos",
"pricing-text": `
<p>Mūsų kainodara – skaidri ir konkurencinga. Jokių paslėptų mokesčių ar netikėtumų.<br><br>
Žemiau pateikiamos apytikslės paslaugų kainos:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Avarinis atrakinimas – nuo €20</li>
  <li>Pamesto rakto sprendimai – nuo €80</li>
  <li>Raktų remontas – nuo €30</li>
  <li>Raktų kopijavimas – nuo €40</li>
  <li>Pulteliai ir uždegimo spynelės remontas – nuo €100</li></strong>
</ul>
<p>Visos kainos apima profesionalų darbą ir garantiją. Norėdami sužinoti tikslią kainą – susisiekite individualiai.</p>
`,

    "contact-us-title": "Susisiekite su mumis",
    "contact-us-text":
      "Dirbame 24/7 Lietuvoje. Susisiekite telefonu, per WhatsApp arba užpildykite formą.",
    footer: "© 2025 RTi Auto Raktai. Visos teisės saugomos.",
    name: "Jūsų vardas",
    email: "El. paštas",
    message: "Jūsų žinutė",
    "send-button": "Siųsti",
    required: "Prašome užpildyti šį laukelį.",  
    invalidEmail: "Prašome įvesti galiojantį el. pašto adresą.",
    thanksTitle: "Ačiū!",
    thanksMsg: "Jūsų žinutė buvo išsiųsta.",
     "nav-chip-tuning": "Čipų programavimas",
    "nav-postal": "Pašto paslauga",
    "nav-about": "Apie mus",
    "nav-services": "Mūsų paslaugos",
    "nav-lockout": "Avarinis automobilio atrakinimas",
    "nav-key-replacement": "Automobilio raktų gamyba",
    "nav-why-us": "Kodėl verta rinktis mus?",
    "nav-ignition": "Užvedimo spynelės remontas",
    "nav-programming": "Automobilių raktų programavimas",
    "nav-pricing": "Kainos ir paslaugos",
    "nav-training": "Mokymai",
    "nav-contact": "Kontaktai",
 "disclaimer-title": "Atsakomybės forma",
    "full-name-label": "Pilnas vardas",
    "email-label": "El. paštas",
    "vehicle-details-label": "Transporto priemonės duomenys",
    "disclaimer-text-label": "Atsakomybės pareiškimas",
    "disclaimer-text": "RTi Auto Raktai neatsako už jokią žalą, atsiradusią po rakto gamybos ar programavimo. Patvirtinu, kad buvau informuotas apie galimą riziką ir atleidžiu įmonę nuo bet kokios atsakomybės.",
    "agree-check": "Sutinku su sąlygomis",
    "submit-button": "Pateikti",
    "thanks-title": "Ačiū!",
    "thanks-text": "Jūsų forma buvo sėkmingai pateikta.",
  },
  
  ru: {
    "hero-title": "Быстрая и надежная помощь с ключами",
    "hero-sub":
      "Автомобили, мотоциклы, грузовики – программирование, копирование, ECU ремап",
    "banner-text": "🔐 Экстренная помощь 24/7 по всей Литве.",
    "hero-phone": "📞 Позвоните +370 623 85036 или напишите в WhatsApp",
"contact-button": "📞Позвонить",
 "contact-aria": "Позвоните нам сейчас",
   "about-title": "О нас",
"about-text": `
<p>RTi Auto Raktai имеет более <strong>10 лет опыта</strong> в сфере замков и ключей.<br><br>
Мы специализируемся на следующих услугах:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Программирование ключей автомобилей</li>
  <li>Перепрошивка ЭБУ и отключение иммобилайзера</li>
  <li>Экстренное вскрытие любых марок и моделей</li></strong><br>
</ul>
<p>Мы предоставляем быстрые и профессиональные услуги для <strong>автомобилей, мотоциклов, фургонов и грузовиков</strong>.<br><br>
Нужна помощь с автоключами? – <strong>на нас можно положиться</strong>.</p>
`,

    "services-title": "Наши услуги",
"service1": "Программирование автомобильных ключей",
"service2": "Изготовление и программирование ключей для мотоциклов",
"service3": "Решения для ключей грузовиков",
"service4": "Аварийное открытие дверей",
"service5": "Программирование ЭБУ и чипов",
"service6": "Удаление и настройка иммобилайзеров",
 motorbikeService: "<strong>Ключи для мотоциклов</strong> <br><br>Программирование и изготовление ключей для Yamaha, Honda, Suzuki, KTM и др. Отключение иммобилайзера, ремонт замков зажигания.",
    truckService: "<strong>Ключи для грузовиков</strong> <br><br>Дублирование ключей, программирование иммобилайзеров, аварийное вскрытие. Работаем с MAN, Volvo, DAF, Mercedes и др.",
    vanService: "<strong>Ключи для микроавтобусов</strong> <br><br>Изготовление и привязка ключей к бортовому компьютеру. Популярные модели: Renault Trafic, VW Transporter, Ford Transit.",
    autoService: "<strong>Ключи для автомобилей</strong> <br><br>Дублирование ключей, программирование иммобилайзеров, аварийное вскрытие. Работаем с BMW, Audi, Volkswagen, Ford, Volvo и др.",

    postalService: `
      <img src="images/mail-global.png" alt="Почтовая услуга" style="max-width: 50px; border-radius: 8px; margin-bottom: 20px;">
      <h2>Обслуживание по почте</h2>
      <p>Мы предлагаем уникальную услугу – <strong>ремонт, программирование замков и изготовление ключей по почте</strong>. Просто отправьте нам замок от автомобиля или мотоцикла – мы его отремонтируем, запрограммируем и вернём обратно.</p>
      <a href="docs/mail-in-form.pdf" target="_blank" class="btn">📄 Скачать форму заказа (PDF)</a>
    `,
    "gallery-title": "Галерея",
    "training-title": "Обучение",
    "training-text": `
RTi Auto Raktai предлагает профессиональное обучение по программированию ключей, дублированию, ECU ремапу и аварийному вскрытию.<br><br>
Наши курсы подходят как для <strong>начинающих</strong>, так и для <strong>опытных авто-слесарей</strong>, желающих углубить свои навыки.<br><br>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li> Практические занятия с реальными автомобилями и оборудованием</li>
  <li> Теория и диагностика (OBD, EEPROM, транспондерные системы)</li>
  <li> Сертификат по завершению курса</li>
  <li> Обучение онлайн или очно (в Великобритании и Литве)</li>
  <li> Языки: литовский, английский, русский, польский</li></strong>
</ul>
<br>
`,
 "chipTuning.title": "Чип-тюнинг",
    "chipTuning.description": "Профессиональная OBD-диагностика автомобилей и мотоциклов.",
    "chipTuning.servicesTitle": "Варианты чип-тюнинга:",
    "chipTuning.stage1": "Stage 1 – увеличение мощности",
    "chipTuning.eco": "Eco tuning – экономия топлива",
    "chipTuning.note": "Работаем с современной техникой и гарантируем результат.",


    "lockout-title": "Аварийное вскрытие автомобиля",
"lockout-text": `
<p>Автомобиль может оказаться запертым в самый неподходящий момент – это случается даже с самыми осторожными.<br><strong>Мы готовы помочь!</strong></p>
<p>Наши квалифицированные специалисты работают <strong>по всей Литве</strong>, быстро реагируя как днём, так и ночью.</p>
<p>Мы используем <strong>безопасные и неразрушающие</strong> методы, подходящие даже для современных автомобилей с высокой степенью защиты.</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Быстрая реакция по всей Литве</li>
  <li>Безопасные, профессиональные методы вскрытия</li>
  <li>Обслуживание автомобилей, фургонов и мотоциклов</li>
  <li>Круглосуточная помощь</li>
  <li>Опытные и подготовленные специалисты</li>
  <li>Обслуживание на нескольких языках</li></strong>
</ul>
`,


    "key-replacement-title": "Замена автомобильного ключа",
"key-replacement-text": `
<p>Потеряли или сломали ключ? RTi Auto Raktai поможет быстро и надежно.<br><br>
Мы предлагаем <strong>замену ключей на месте</strong> любого типа:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li> <strong>Ключи с пультом</strong></li>
  <li> <strong>Смарт</strong> и бесконтактные ключи</li>
  <li> <strong>Транспондерные</strong> ключи</li>
  <li> <strong>Ретро</strong> и классические ключи</li><br><strong>
</ul>
<p>Наши мастера могут <strong>вырезать, клонировать и запрограммировать</strong> ключ на месте – у вас дома, на работе или на дороге.<br><br>
Все услуги включают <strong>гарантию</strong> и <strong>программирование противоугонного чипа</strong>.</p>
`,

    "why-us-title": "Почему выбирают нас?",
"why-us-text": `
<p>Мы – не просто мастера по замкам, мы – ваш надежный партнёр по автомобильной безопасности.<br><br>
RTi Auto Raktai предлагает <strong>высококачественный сервис</strong> по <strong>доступным ценам</strong> в вашем районе.</p>

<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li> Сотни 5-звездочных отзывов</li>
  <li> Прозрачные и конкурентные цены – без скрытых платежей</li>
  <li> Выездные специалисты по всему Северо-Западу Великобритании</li>
  <li> Круглосуточная помощь 24/7</li>
  <li> Работаем со всеми марками и моделями</li>
  <li> Вежливое, профессиональное и многоязычное обслуживание</li></strong>
</ul>

<p>Выбирая нас, вы получаете <strong>уверенность</strong>, <strong>оперативность</strong> и <strong>гарантированный результат</strong>.</p>
`,

   "ignition-title": "Ремонт замка зажигания",
"ignition-text": `
<p>Система зажигания — одна из самых важных и чувствительных частей автомобиля.<br><br>
RTi Auto Raktai предлагает профессиональный сервис:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Ремонт изношенных или повреждённых замков зажигания</li>
  <li>Замена неисправных механизмов</li>
  <li>Извлечение сломанных ключей из замка</li></strong>
</ul>
<p><strong>Не пытайтесь ремонтировать самостоятельно</strong> — это может повредить систему.<br><br>
Доверьтесь нашим опытным специалистам — мы восстановим зажигание быстро и безопасно.</p>
`,

   "programming-title": "Программирование автоключей",
"programming-text": `
<p>Современные автомобили требуют точного и безопасного программирования ключей. В RTi Auto Raktai мы используем передовые технологии и оборудование.<br><br>
Наши услуги программирования включают:</p>

  <li>Программирование через OBD</li>
  <li>Программирование чипов на уровне EEPROM</li>
  <li>Сопряжение транспондерных ключей</li>
  <li>Синхронизация с ECU и иммобилайзером</li><br><strong>
</ul>
<p>Будь то запасной ключ или полная интеграция – <strong>мы гарантируем точность и надежность</strong>.</p>
`,

    "pricing-title": "Цены и услуги",
"pricing-text": `
<p>Мы предлагаем прозрачные и конкурентные цены – никаких скрытых платежей.<br><br>
Примерная стоимость наших услуг:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Аварийное вскрытие – от €20</li>
  <li>Услуги при потере ключа – от €80</li>
  <li>Ремонт ключей – от €30</li>
  <li>Копирование ключей – от €40</li>
  <li>Пульты и ремонт замка зажигания – от €100</li></strong>
</ul>
<p>Все цены включают гарантию и профессиональный сервис. Свяжитесь с нами для точной оценки по вашему авто.</p>
`,

    "contact-us-title": "Связаться с нами",
    "contact-us-text":
      "Работаем 24/7 в. Позвоните, напишите в WhatsApp или заполните форму.",
    "contact-title": "Контакты",
    "uk-contact": "📍 Литва,район Аникщяй ",

    footer: "© 2025 RTi Auto Raktai. Все права защищены.",
    name: "Ваше имя",
    email: "Эл. почта",
    message: "Ваше сообщение",
    "send-button": "Отправить",
    required: "Пожалуйста, заполните это поле.",
    invalidEmail: "Пожалуйста, введите действующий адрес электронной почты.",
    thanksTitle: "Спасибо!",
    thanksMsg: "Ваше сообщение отправлено.",
    "nav-chip-tuning": "Чип-тюнинг",
    "nav-postal": "Услуга по почте",
    "nav-about": "О нас",
    "nav-services": "Наши услуги",
    "nav-lockout": "Аварийное открытие автомобиля",
    "nav-key-replacement": "Изготовление автоключей",
    "nav-why-us": "Почему выбирают нас?",
    "nav-ignition": "Ремонт замка зажигания",
    "nav-programming": "Программирование автоключей",
    "nav-pricing": "Цены и услуги",
    "nav-training": "Обучение",
    "nav-contact": "Контакты",
"disclaimer-title": "Форма отказа от ответственности",
"full-name-label": "Полное имя",
"vehicle-details-label": "Данные транспортного средства",
"disclaimer-text": "RTi Auto Raktai не несет ответственности за неисправности, возникшие после программирования или изготовления ключа.",
"agree-check": "Я согласен с условиями",
"submit-button": "Отправить",
"thanks-title": "Спасибо!",
"thanks-text": "Ваша форма отказа от ответственности успешно отправлена.",
"send-button": "Отправить",
 "disclaimer-title": "Форма отказа от ответственности",
    "full-name-label": "Полное имя",
    "email-label": "Эл. почта",
    "vehicle-details-label": "Данные транспортного средства",
    "disclaimer-text-label": "Отказ от ответственности",
    "disclaimer-text": "RTi Auto Raktai не несет ответственности за любые проблемы, возникшие после изготовления или программирования ключа. Я подтверждаю, что был проинформирован о возможных рисках и освобождаю компанию от всей ответственности.",
    "agree-check": "Я согласен с условиями",
    "submit-button": "Отправить",
    "thanks-title": "Спасибо!",
    "thanks-text": "Ваша форма успешно отправлена."

  },
  pl: {
    "hero-title": "Szybka i niezawodna pomoc z kluczami",
    "hero-sub":
      "Samochody, motocykle, ciężarówki – programowanie, kopiowanie, remap ECU",
    "banner-text": "🔐 Całodobowa pomoc awaryjna na terenie całej Litwy.",
 "hero-phone": "📞 Zadzwoń +370 623 85036 lub napisz na WhatsApp",
    "contact-button": "📞Zadzwoń",
     "contact-aria": "Zadzwoń do nas teraz",
    "about-title": "O nas",
"about-text": `
<p>RTi Auto Raktai ma ponad <strong>10 lat doświadczenia</strong> w dziedzinie zamków i kluczy.<br><br>
Specjalizujemy się w usługach takich jak:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Programowanie kluczy samochodowych</li>
  <li>Remap ECU i wyłączenie immobilizera</li>
  <li>Awaryjne otwieranie wszystkich marek i modeli</li></strong><br>
</ul>
<p>Oferujemy szybkie i profesjonalne usługi dla <strong>samochodów, motocykli, dostawczych i ciężarówek</strong>.<br><br>
Potrzebujesz pomocy z kluczem? – <strong>możesz na nas polegać</strong>.</p>
`,


    "services-title": "Nasze usługi",
"service1": "Programowanie kluczy samochodowych",
"service2": "Dorabianie i programowanie kluczy do motocykli",
"service3": "Rozwiązania dla kluczy ciężarówek i vanów",
"service4": "Awaryjne otwieranie drzwi",
"service5": "Programowanie ECU i chipów",
"service6": "Usuwanie i konfiguracja immobilizerów",
    motorbikeService: "<strong>Klucze do motocykli</strong> <br><br>Programowanie i produkcja kluczy do Yamaha, Honda, Suzuki, KTM i innych. Wyłączenie immo, naprawa stacyjek.",
    truckService: "<strong>Klucze do ciężarówek</strong> <br><br>Dublowanie kluczy, programowanie immobilizera, awaryjne otwieranie. Obsługujemy MAN, Volvo, DAF, Mercedes i inne.",
    vanService: "<strong>Klucze do mikrobusów</strong> <br><br>Produkcja i przypisywanie kluczy do komputera pokładowego. Popularne modele: Renault Trafic, VW Transporter, Ford Transit.",
    autoService: "<strong>Klucze do samochodów</strong> <br><br>Dublowanie kluczy, programowanie immobilizera, awaryjne otwieranie. Obsługujemy BMW, Audi, Volkswagen, Ford, Volvo i inne.",
    postalService: `
    <img src="images/mail-global.png" alt="Usługa pocztowa" style="max-width: 50px; border-radius: 8px; margin-bottom: 20px;">
    <h2>Usługa pocztowa</h2>
    <p>Oferujemy unikalną usługę – <strong>naprawę, programowanie zamków i dorabianie kluczy przez wysyłkę</strong>. Wystarczy wysłać do nas zamek samochodowy lub motocyklowy – naprawimy go, zaprogramujemy i odeślemy z powrotem.</p>
    <a href="docs/mail-in-form.pdf" target="_blank" class="btn">📄 Pobierz formularz zamówienia (PDF)</a>`,
    "gallery-title": "Galeria",
    "contact-title": "Kontakt",
   "uk-contact": "📍 Litwa,rejon Anykščiai",

    "training-title": "Szkolenia",
    "training-text": `
RTi Auto Raktai oferuje profesjonalne szkolenia z zakresu programowania kluczy, kopiowania, remapu ECU i awaryjnego otwierania pojazdów.<br><br>
Nasze kursy są przeznaczone zarówno dla <strong>początkujących</strong>, jak i <strong>doświadczonych ślusarzy samochodowych</strong>, którzy chcą poszerzyć swoje umiejętności.<br><br>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li> Zajęcia praktyczne na prawdziwych pojazdach i sprzęcie</li>
  <li> Teoria i diagnostyka (OBD, EEPROM, systemy transponderowe)</li>
  <li> Certyfikat po ukończeniu</li>
  <li> Szkolenia dostępne online lub stacjonarnie (UK i Litwa)</li>
  <li> Języki: litewski, angielski, rosyjski, polski</li></strong>
</ul>
<br>
`,
 "chipTuning.title": "Chip Tuning",
    "chipTuning.description": "Profesjonalna diagnostyka komputerowa OBD dla samochodów i motocykli.",
    "chipTuning.servicesTitle": "Dostępne opcje chip tuningu:",
    "chipTuning.stage1": "Stage 1 – zwiększenie mocy",
    "chipTuning.eco": "Eco tuning – oszczędność paliwa",
    "chipTuning.note": "Używamy nowoczesnego sprzętu i zapewniamy jakość usług.",
 "lockout-title": "Awaryjne otwieranie samochodu",
"lockout-text": `
<p>Zatrzaśnięte drzwi samochodu? To może przydarzyć się każdemu – zazwyczaj w najmniej odpowiednim momencie.<br><strong>Jesteśmy gotowi pomóc!</strong></p>
<p>Nasi wykwalifikowani technicy działają <strong>na terenie całej Litwy</strong>, zapewniając szybką pomoc o każdej porze dnia i nocy.</p>
<p>Stosujemy <strong>bezpieczne i bezinwazyjne</strong> metody otwierania, odpowiednie również dla nowoczesnych pojazdów z zaawansowanymi systemami zabezpieczeń.</p>
<ul>
  <li>Szybka reakcja na terenie całej Litwy</li>
  <li>Profesjonalne, nieinwazyjne metody</li>
  <li>Samochody osobowe, dostawcze i motocykle</li>
  <li>Pomoc 24/7</li>
  <li>Doświadczeni i dobrze wyposażeni specjaliści</li>
  <li>Obsługa w kilku językach</li>
</ul>
`,

    "key-replacement-title": "Wymiana klucza samochodowego",
"key-replacement-text": `
<p>Zgubiłeś lub uszkodziłeś klucz? RTi Auto Raktai pomoże Ci szybko wrócić na drogę.<br><br>
Oferujemy <strong>wymianę kluczy na miejscu</strong> dla każdego typu pojazdu:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li> <strong>Klucze z pilotem</strong> (remote)</li>
  <li> <strong>Smart</strong> i bezdotykowe klucze</li>
  <li> <strong>Transponderowe</strong> klucze</li>
  <li> <strong>Retro</strong> i klasyczne klucze</li><br><strong>
</ul>
<p>Nasi specjaliści mogą <strong>wyciąć, sklonować i zaprogramować</strong> klucz na miejscu – w domu, pracy lub na poboczu.<br><br>
Wszystkie usługi obejmują <strong>gwarancję</strong> oraz <strong>programowanie układu antykradzieżowego</strong>.</p>
`,

    "why-us-title": "Dlaczego warto nas wybrać?",
"why-us-text": `
<p>Nie jesteśmy tylko ślusarzami – jesteśmy Twoim zaufanym partnerem w zakresie bezpieczeństwa pojazdów.<br><br>
RTi Auto Raktai łączy <strong>jakość usług</strong> z <strong>przystępnymi cenami</strong> i szybką dostępnością.</p>

<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li> Setki ocen 5-gwiazdkowych od zadowolonych klientów</li>
<li> Mobilni technicy w północno-wschodniej Litwie</li>
  <li> Przejrzyste, konkurencyjne ceny – bez ukrytych opłat</li>
  
  <li> Pomoc 24/7 przez całą dobę</li>
  <li> Obsługa wszystkich marek i modeli</li>
  <li> Profesjonalna, miła i wielojęzyczna obsługa</li></strong>
</ul>

<p>Wybierając nas, zyskujesz <strong>spokój ducha</strong>, <strong>szybką pomoc</strong> i <strong>sprawdzone rezultaty</strong>.</p>
`,

    "ignition-title": "Naprawa stacyjek zapłonowych na Litwie",

"ignition-text": `
<p>Stacyjka to jeden z najbardziej wrażliwych i kluczowych elementów pojazdu.<br><br>
W RTi Auto Raktai oferujemy profesjonalną pomoc w zakresie:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Naprawy zużytych lub uszkodzonych zamków zapłonu</li>
  <li>Wymiany niesprawnych mechanizmów</li>
  <li>Usuwanie złamanych kluczy ze stacyjek zapłonowych na Litwie</li></strong>

</ul>
<p><strong>Nie próbuj naprawiać samodzielnie</strong> – możesz pogorszyć sytuację.<br><br>
Zaufaj naszym fachowcom, którzy przywrócą sprawność Twojej stacyjce szybko i bezpiecznie.</p>
`,

    "programming-title": "Programowanie kluczy samochodowych",
"programming-text": `
<p>Nowoczesne pojazdy wymagają zaawansowanej technologii do programowania kluczy. W RTi Auto Raktai korzystamy z profesjonalnych narzędzi, aby zapewnić precyzję i bezpieczeństwo.<br><br>
Nasze usługi obejmują:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Programowanie przez złącze OBD</li>
  <li>Programowanie chipów metodą EEPROM</li>
  <li>Parowanie kluczy transponderowych</li>
  <li>Synchronizację z ECU i immobilizerem</li></strong>
</ul>
<p>Niezależnie czy potrzebujesz zapasowego klucza, czy pełnej integracji – <strong>zapewniamy dokładność i skuteczność</strong>.</p>
`,

    "pricing-title": "Cennik usług",
"pricing-text": `
<p>Nasze ceny są przejrzyste i uczciwe – bez ukrytych kosztów.<br><br>
Przykładowe stawki naszych usług:</p>
<ul style="margin-left: 4rem; list-style-position: outside;"><br><strong>
  <li>Awaryjne otwieranie – od €20</li>
  <li>Rozwiązania przy utracie klucza – od €80</li>
  <li>Naprawa kluczy – od €30</li>
  <li>Dorabianie kluczy – od €40</li>
<li>Piloty i naprawa stacyjek zapłonowych – od €100</li></strong>

</ul>
<p>Wszystkie usługi obejmują gwarancję i profesjonalne wykonanie. Skontaktuj się z nami, aby uzyskać indywidualną wycenę.</p>
`,

    "contact-us-title": "Skontaktuj się z nami",
    "contact-us-text":
     "Dostępni 24/7 na terenie Litwy. Zadzwoń, napisz na WhatsApp lub wypełnij formularz.",

    footer: "© 2025 RTi Auto Raktai. Wszelkie prawa zastrzeżone.",
    name: "Twoje imię",
    email: "Twój email",
    message: "Twoja wiadomość",
    "send-button": "Wyślij",
    required: "Proszę wypełnić to pole.",
    invalidEmail: "Proszę podać poprawny adres e-mail.",  
    thanksTitle: "Dziękujemy!",
    thanksMsg: "Twoja wiadomość została wysłana.",
   "nav-chip-tuning": "Chip tuning",
    "nav-postal": "Usługa pocztowa",
    "nav-about": "O nas",
    "nav-services": "Nasze usługi",
    "nav-lockout": "Awaryjne otwieranie samochodu",
    "nav-key-replacement": "Dorabianie kluczy samochodowych",
    "nav-why-us": "Dlaczego my?",
    "nav-ignition": "Naprawa stacyjki",
    "nav-programming": "Programowanie kluczy",
    "nav-pricing": "Cennik i usługi",
    "nav-training": "Szkolenia",
    "nav-contact": "Kontakt",
"disclaimer-title": "Formularz zrzeczenia się odpowiedzialności",
"full-name-label": "Imię i nazwisko",
"vehicle-details-label": "Dane pojazdu",
"disclaimer-text": "RTi Auto Raktai nie ponosi odpowiedzialności za usterki powstałe po zaprogramowaniu lub wykonaniu klucza.",
"agree-check": "Zgadzam się z warunkami",
"submit-button": "Wyślij",
"thanks-title": "Dziękujemy!",
"thanks-text": "Twój formularz zrzeczenia się odpowiedzialności został pomyślnie wysłany.",
"send-button": "Wyślij",
"disclaimer-title": "Formularz zrzeczenia się odpowiedzialności",
    "full-name-label": "Imię i nazwisko",
    "email-label": "E-mail",
    "vehicle-details-label": "Dane pojazdu",
    "disclaimer-text-label": "Oświadczenie",
    "disclaimer-text": "RTi Auto Raktai nie ponosi odpowiedzialności za jakiekolwiek usterki po wykonaniu lub zaprogramowaniu klucza. Potwierdzam, że zostałem poinformowany o możliwych zagrożeniach i zwalniam firmę z wszelkiej odpowiedzialności.",
    "agree-check": "Zgadzam się z warunkami",
    "submit-button": "Wyślij",
    "thanks-title": "Dziękujemy!",
    "thanks-text": "Twój formularz został pomyślnie wysłany."

  },
};
